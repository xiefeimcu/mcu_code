...
{6,4,"通信电源",SetDTUPower,Null,(struct Menu*)MainMenu},
{6,4,"通信预热",SetDTUPreheat,Null,(struct Menu*)MainMenu},
...
/*******************************************************************************
函数名称：void ShowMenu(Menu *MenuUser,unsigned char DisplayPoint,unsigned char refresh)
函数介绍：显示菜单
...
void SetMenu(void)
{
  ...
  unsigned char i=0;                    //菜单层级
  signed char UserChoose = 0;           //当前用户选择
  signed char USER=0;                   //前一次用户选择 
  unsigned char  DisplayPoint = 0;      //显示点
  unsigned char  RefreshMenu = 1;       //刷新菜单标志
...
/*******************************************************************************
函数名称：long int SetNum(setnum num,unsigned char H,unsigned char Ver)
函数介绍：设置数字
...
/*******************************************************************************
函数名称：unsigned char SetString(setstring s,unsigned char H,unsigned char Ver)
函数介绍：选择字符
...
unsigned char SetYaoSu(unsigned char ys,unsigned char H,unsigned char Ver)
{
  ...
  {   
    ...
    switch(System.Key)
    {
      case Up:  Draw(H,Ver,strlenght(YaoSuInfo[ys].NameStr),16,0,Null);
                ys++;
                if(ys > YAOSULENG-1)
                  ys = 0;               
                break;
      case Down: Draw(H,Ver,strlenght(YaoSuInfo[ys].NameStr),16,0,Null);
                if(0==ys)
                   ys = YAOSULENG-1; 
                 else
                   ys--;                                            
...
{  
  unsigned char temp[5]={'P','H','K','Z','M'};//遥测站分类码
  unsigned char i=0;
  setstring settemp={0,{"降水","河道","水库","闸坝","墒情",""}};
  
  DisplayString(4,8,"测站类型",0);
  H_LINE(2,49,12);
  for(i=0;i<5;i++)//参数超出范围恢复为 降水
  {
    if(RTU.RTUType==temp[i])
      break;
  }
  if(i >= 5)
    i = 0;
  ...
  RTU.RTUType = temp[settemp.user];
  WriteUInt8(RTU.RTUType,EEPROM.RTUtype);//保存设置值
}

void SetCenteradd(void)
{
  setnum num={RTU.CenterAdd,0X30,0,""}; 
  num.max = 255;
  num.min = 0; 
  
  DisplayString(3,8,"中心站地址",0);
  H_LINE(2,49,12);
  RTU.CenterAdd = (unsigned char)SetNum(num,6,32); 
  WriteUInt8(RTU.CenterAdd,EEPROM.CenterAdd);//保存设置值 
}

void SetST(void)
{
  unsigned char strtemp[11]={0};
  unsigned char i=0,j=0;
  unsigned int fb=0;
  ...
  while(1)
  {
    ...
    WaitKey();//等待按键按下
      ...
      switch(System.Key)
      {
        case Up: if(++strtemp[i]>'9')
                   strtemp[i] = '0';
                 break;
        case Down: if(--strtemp[i]<'0')
                   strtemp[i] = '9';
                 break;  
        case Right: if(++i > 9)
                      i = 0;
...
{  
  unsigned char temp[4]={WMODE_1,WMODE_2,WMODE_3,WMODE_4};
  unsigned char i=0;
...
{
  unsigned char temp[4]={0,5,10,30};
  unsigned char i=0;
...
void settimetype(unsigned char type,unsigned char H,unsigned char Ver)
{
  ...
  unsigned char  i;             //设置位选择  
  unsigned char  max;           //能够设置的最大值
  unsigned char  min;           //能够设置的最小值
  unsigned char  vale;          //设置项当前值
  unsigned char  valetemp;      //设置项值改变后存储
...
{
  unsigned char i;
  unsigned char UserChoose=1;  //用户选择
