...
/*写字节*/
void SPI_HostWriteByte(char wByte) 
{ 
  char i; 
  for(i = 0; i < 8; i++) 
  { 
...
参数：             
   ...
   len   - 指定数据的长度          
*******************************************************************************/ 
void Read_Buffer(char buffer, unsigned int BFA, char *pHeader, unsigned int len) 
{
  unsigned int i = 0; 
  ...
  SPI_HostWriteByte((char)(BFA >> 8));//the don't care 15bits of low seven bits and the BFA of high one bit
  SPI_HostWriteByte((char)BFA);   //the BFA of low eight bits
...
参数：             
   ...
   len   - 指定数据的长度          
******************************************************************************/ 
void AT45DB041B_ContinuousArrayRead(unsigned int PA, unsigned int BFA, char *pHeader, unsigned int len) 
{   
  unsigned int i = 0;  
  ...
  SPI_HostWriteByte((char)(PA >> 7));//此处代码很妙啊,PA >> 7则是发送PA的高4位以及最开始的4位无关紧要位   
  SPI_HostWriteByte((char)((PA << 1) | (BFA >> 8)));//因为PA有2个字节共16位,但是PA只用到了第1位到11位,
  SPI_HostWriteByte((char)BFA);//PA左移一位再或上BFA >> 8则是发送PA的低7位和BFA最高位,最后再发送BFA的低8位
  for(i = 0; i < 4; i++) 
  {
...
参数：             
   ...
   len   - 指定数据的长度          
******************************************************************************/
void AT45DB041B_MainMemoryPageRead(unsigned int PA, unsigned int BFA, char *pHeader, unsigned int len) 
{   
  unsigned int i = 0;
...
参数：            
  ...
  在该指令序列中，操作码84H选择BUFFER 1，87H选择BUFFER 2   
   ...
   len   - 待存数据的长度1~264        
******************************************************************************/ 
void AT45DB041B_BufferWrite(char buffer, unsigned int BFA, char *pHeader, unsigned int len) 
{ 
  unsigned int i = 0; 
  ...
  SPI_HostWriteByte(0x00); 
  SPI_HostWriteByte((char)(BFA >> 8)); 
  SPI_HostWriteByte((char)BFA);  
  for(i = 0; i < len; i++) 
  {
...
{ 
  ...
  SPI_HostWriteByte((char)(PA >> 7)); 
  SPI_HostWriteByte((char)(PA << 1)); 
  SPI_HostWriteByte(0x00);  
  Flash_CS_H;
} 
...
描述：            
   与上一个函数的唯一区别是不带预擦除。        
******************************************************************************/ 
void AT45DB041B_BufferToMainMemoryPageProgramWithoutBuilt_inErase(char buffer, unsigned int PA, unsigned int BFA, char *pHeader, unsigned int len)
{ 
  unsigned int i = 0; 
...
void AT45DB041B_PageErase(unsigned int PA) 
{
  ...
  SPI_HostWriteByte((char)(PA >> 7));//页地址高4位和4为无关位
  SPI_HostWriteByte((char)(PA << 1));//页地址低7位和1位无关位
...
参数：                 
   BLKA   - 块地址，0~255                  
******************************************************************************/
void AT45DB041B_BlockErase(char BLKA) 
{
  unsigned int i = 0; 
  ...
  SPI_HostWriteByte((char)(BLKA >> 4));//块地址高4位和4为无关位
  SPI_HostWriteByte((char)(BLKA << 4));//块地址低4位和4位无关位
  SPI_HostWriteByte(0x00);//8位无关位
  Flash_CS_H;
  while(!(AT45DB041B_StatusRegisterRead() & 0x80));//等待擦除完毕
}
...
参数：                 
  ...
  PA  - 页地址，0~2047    
******************************************************************************/
void Flash_to_buffer(char buffer, unsigned int PA) 
{
  unsigned int i = 0;
...
参数：                 
  ...
  PA  - 页地址，0~2047                
******************************************************************************/
char AT45DB041B_MainMemoryPageToBufferCompare(char buffer, unsigned int PA) 
{
  unsigned int i = 0; 
...
参数：                 
  ...
  len   - 写入数据的长度             
******************************************************************************/
void AT45DB041B_MainMemoryPageProgramThroughBuffer(char buffer, unsigned int PA, unsigned int BFA, char *pHeader, unsigned int len) 
{ 
  unsigned int i = 0; 
  ...
  SPI_HostWriteByte((char)(PA >> 7)); 
  SPI_HostWriteByte((char)(PA << 1 | BFA >> 8)); 
  SPI_HostWriteByte((char)BFA);
  for(i = 0; i < len; i++) 
  {
...
参数：                 
  ...
  len   - 写入数据的长度             
******************************************************************************/
void AT45DB041B_AutoPageRewrite(char buffer, unsigned int PA) 
{
  unsigned int i = 0;
