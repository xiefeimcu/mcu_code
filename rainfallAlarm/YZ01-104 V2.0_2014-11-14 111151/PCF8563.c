...
**------------------------------------------------------------------------------------------------------
********************************************************************************************************/
char PCF8563ReadDatas(char address,char  *p,char count)
{
    char i;
...
char PCF8563Ini(void)
{
  ...
  PCF8563writeData(0x01,0x00);		 //关闭报警中断
  PCF8563writeData(0x09,0x80);		 
...
/*************将时钟BCD码转换为8bit整型************/
    dt.Year = BCD_Byte(timebcd[6]); //年
    dt.Month = BCD_Byte(timebcd[5]);//月
    dt.Day = BCD_Byte(timebcd[3]);         
    dt.Hour = BCD_Byte(timebcd[2]);
    dt.Minute = BCD_Byte(timebcd[1]);
    dt.Second = BCD_Byte(timebcd[0]);
    
    if(TimeCheck(dt))                  //读出的时间合法
    {
      t->Year = dt.Year;
      t->Month = dt.Month;
      t->Day = dt.Day;
